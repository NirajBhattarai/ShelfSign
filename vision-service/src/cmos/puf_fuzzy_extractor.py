...
def pick_bch_params(
    assumed_worst_flips: int, candidate_bits: int = 256, target_margin: float = 2.5
) -> BCHParams:
...
def enroll(
    measured: bytes, params: BCHParams, *, with_crc: bool = True
) -> EnrollResult:
    """measured: raw extracted PUF bit-buffer (>= params.buffer_bytes bytes).

    When with_crc=True (default for new enrollments), reserves the last 2 bytes
    of the BCH message for a CRC-16 over the random payload so regenerate can
    reject silent miscorrections.
    """
    ...
    if with_crc and params.data_bytes < 3:
        raise ValueError(
            f"data_bytes={params.data_bytes} too small for CRC-protected payload"
        )
    measured_buf = measured[: params.buffer_bytes]

    bch = bchlib.BCH(params.t, m=params.m)
    if with_crc:
        payload = bytearray(os.urandom(params.data_bytes - 2))
        crc = _crc16_ccitt(payload)
        random_data = bytearray(payload) + bytearray(crc.to_bytes(2, "big"))
    else:
...
def regenerate(
    ...
    *,
    ...
    error pattern (i.e. more than params.t bits flipped) or, when with_crc,
    if the CRC check fails (silent miscorrection to a wrong codeword)."""
    ...
    if with_crc:
        ...
        crc_got = int.from_bytes(bytes(data[-2:]), "big")
        if _crc16_ccitt(payload) != crc_got:
            raise RuntimeError(
                f"BCH miscorrection detected: CRC mismatch after nerr={nerr} "
                f"(true bit flips likely exceeded t={params.t})."
            )
...
def _crc16_ccitt(data: bytes) -> int:
    """CRC-16/CCITT-FALSE (poly 0x1021, init 0xFFFF) — compact integrity tag
    inside the BCH message so wrong-codeword decodes fail closed."""
    crc = 0xFFFF
    for byte in data:
        crc ^= byte << 8
        for _ in range(8):
